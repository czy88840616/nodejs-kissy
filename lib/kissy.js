...
browser = jsdom.windowAugmentation(jsdom.defaultLevel, {
	parser: parser
});
...
(function(win, S, undefined) {
    ...
    var doc = win['document'], loc = location,
        ...
        //isReady = false,
        isReady = true,//nodejs-kissy 不需要DomReady
				...
				if (err) {
					...
					if (err.stack) {
						S.each(err.stack.split('\n'), function(frame) {
							S.log(frame, 'error', 'KISSY');
						});
					} else {
						S.log(getErrorMessage(err), 'error', 'KISSY');
						S.log('In file: ' + getErrorFilename(err), 'error', 'KISSY');
						S.log('On line: ' + getErrorLine(err), 'error', 'KISSY');
					}
					S.log('----------------------------------------------------------', 'error', 'KISSY');
				} else {
					//S.log(cb);
...
KISSY.log = function(msg, cat, src) {
	var cat = cat || '',color = 37,
		consoleColor = function(str, num) {
			if (!num) {
				num = '37';
			}
	...
	switch(cat){
		case 'info':
			...
			break;
		case 'warn':
			color = contentColor = 33;
			break;
		case 'error':
			color = contentColor = 31;
			break;
		case 'time':
		case 'debug':
			color = 36;
	...
	src = (src) ? consoleColor(' (' +  src.toLowerCase() + '):', 35) : '';

	if (KISSY.Config.debug) {
        sys.error(consoleColor(cat.toLowerCase() + ':', color+';40') + src + ' ' + consoleColor(msg,contentColor));
	}
};

//return KISSY
KISSY.Browser = browser;
exports.KISSY = KISSY;
